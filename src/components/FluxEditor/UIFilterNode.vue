<template>
  ...
  <!--OUT-->
  <Handle v-for="(key, idx) in typeData.handles.out" :key="`s-${key}`" :id="`source-${props.id}-${key}`" type="source"
    :position="Position.Right" :connectable="uniqueCollectHandler(`source-${props.id}-${key}`, null)"
    :style="{ top: `calc(${(idx + 0.5) * getUnit(typeData.handles.out)}% - 10px)`, height: '20px' }" />

  <!--IN-->
  <Handle v-for="(key, idx) in typeData.handles.in" :key="`t-${key}`" :id="`target-${props.id}-${key}`" type="target"
    :position="Position.Left" :connectable="uniqueCollectHandler(null, `target-${props.id}-${key}`)"
